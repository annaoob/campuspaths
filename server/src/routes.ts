...
import { LocationTree, buildTree, findClosestInTree } from "./location_tree";
...
export const getShortestPath = (req: SafeRequest, res: SafeResponse): void => {
  ...
  if (friends !== undefined) {
    ...
    for (const friend of friends) {
      ...
      // build friend's location tree
      const fTree: LocationTree = buildTree(fLocs);
      // find closest location in friend's location tree to user's locs
      const fLoc = findClosestInTree(fTree, locs);
      // load that friend and their closest location into nearby array
      nearby.push({friend: friend, dist: fLoc[1], loc: fLoc[0]});
    }
  }
