...
import { parseNearbyList, Nearby } from './nearby';
...
type MapState = {
  ...
  hour?: Hour;
  path?: Array<Edge>;
  points?: Array<Nearby>;  // add nearby points in state to keep track of closest locs of friends
};
...
export class MapViewer extends Component<MapProps, MapState> {
  ...
  renderEndPoints = (): Array<JSX.Element> => {
    ...
    // if there are nearby friends
    if (this.state.points !== undefined) {
      // for every friend
      for (const friend of this.state.points) {
        // if the friend is within a 300 readius
        if (friend.dist <= 300) {
          // make their circle color be the the color stored in FRIEND_COLORS at friend's index in points mod 6
          elems.push(<circle cx={friend.loc.x} cy={friend.loc.y} fill={FRIEND_COLORS[this.state.points.indexOf(friend) % 6]} 
          r={RADIUS} stroke={'white'} strokeWidth={10} key={'start'}/>);
        }
      }
    }
  ...
  renderLegendItems = (): Array<JSX.Element> => {
    ...
    if (this.state.points !== undefined) {
      // for each nearby friend
      for (const friend of this.state.points) {
        // if the friend is within a 300 radius
        if (friend.dist <= 300) {
          // make their circle color be the the color stored in FRIEND_COLORS at friend's index in points mod 6
          // this will match with their color on the map because the color on the map is determined in a logically identical way
          // and if this.state.points changes, the color would get updated identically on both the map and legend
          items.push(makeLegendItem(FRIEND_COLORS[this.state.points.indexOf(friend) % 6], `${friend.friend} is nearby`, `friend ${this.state.points.indexOf(friend)}`));
        }
      }
    }
  ...
  doShortestPathJson = (data: unknown): void => {
    ...
    if (data.found)
      ...
      // parse & record the nearby points in the state
      this.setState({
        path: parseEdges(data.path),
        points: parseNearbyList(data.nearby)
      });
  }
